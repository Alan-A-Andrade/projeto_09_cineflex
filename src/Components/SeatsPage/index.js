  ...
  return(
    <div className='seats-page'>
        ...
        <div className="seats-list">
            {sessionData.seats.map(el => 
                <Seat   
                    onClick={selectSeat} 
                    seatId={el.id} 
                    key={`${sessionId}_${el.name}`} 
                    name={el.name} 
        ...
        <div className="buy-button" >
            {renderInputBox.length === 0 ? 
            <h1 style={{backgroundColor: "grey", opacity: 0.5}}>Reservar assento(s)</h1> :
            <h1 onClick={()=>{buyTickets()}}>Reservar assento(s)</h1>}
        </div>
